...
const db = {
    ...
    getServiceOrders: () => serviceOrdersDb,
    addServiceOrder: (order) => {
        const client = clientsDb.find(c => c.id === order.clientId);
        if (!client) return { error: "Cliente não encontrado" };
        const newOrder = { ...order, id: `os-${Date.now()}`, clientName: client.razaoSocial || client.nomeCompleto || 'Cliente Desconhecido', status: OSStatus.Pending, };
        serviceOrdersDb.unshift(newOrder);
        return { order: newOrder };
    },
    getUsers: () => usersDb,
    addUser: (user) => {
        const newUser = { ...user, id: `user-${Date.now()}`, status: 'Ativo' };
        if (newUser.username && user.password) {
            USER_PASSWORDS.set(newUser.username, user.password);
        }
        delete newUser.password; // Never store plain text password
    ...
    addQuote: (quoteData) => {
        ...
        if (!client) return { error: "Cliente não encontrado" };
        const newQuote = { ...quoteData, id: `qt-${Date.now()}`, quoteNumber: `ORC-${String(quotesDb.length + 1).padStart(4, '0')}`, clientName: client.razaoSocial || client.nomeCompleto || 'Cliente Desconhecido', status: QuoteStatus.Draft, };
...
// Clients
apiRouter.get('/clients', (req, res) => res.json(db.getClients()));
apiRouter.post('/clients', (req, res) => res.status(201).json(db.addClient(req.body)));

// Service Orders
apiRouter.get('/service-orders', (req, res) => res.json(db.getServiceOrders()));
apiRouter.post('/service-orders', (req, res) => {
    const { order, error } = db.addServiceOrder(req.body);
...
apiRouter.post('/users', (req, res) => res.status(201).json(db.addUser(req.body)));

// Products
apiRouter.get('/products', (req, res) => res.json(db.getProducts()));
apiRouter.post('/products', (req, res) => res.status(201).json(db.addProduct(req.body)));

// Quotes
apiRouter.get('/quotes', (req, res) => res.json(db.getQuotes()));
apiRouter.post('/quotes', (req, res) => {
    const { quote, error } = db.addQuote(req.body);
    if (error) return res.status(400).json({ message: error });
